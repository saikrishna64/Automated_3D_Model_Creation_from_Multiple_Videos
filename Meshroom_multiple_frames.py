...
os.environ["ALICEVISION_ROOT"] = "C:\\Users\\kesan\\Downloads\\Meshroom-2023.2.0-win64\\Meshroom-2023.2.0\\aliceVision"
...
def process_folder(binPath, baseDir, imgDir):
...
def main():
    binPath = "C:\\Users\\kesan\\Downloads\\Meshroom-2023.2.0-win64\\Meshroom-2023.2.0\\aliceVision\\bin"  ## path of the binary files from Meshroom

    # List of folders containing the collections of photos
    folder_path = "D:\\IndeVFX\pythonProject\\4D_point_cloud\\Meshroom_models\\Sample_Frames"  # You can replace this with the desired directory path
    imgDirs = list(filter(lambda x: os.path.isdir(x),
                                 [os.path.join(folder_path, item) for item in os.listdir(folder_path)]))
    print(imgDirs)

    for idx, imgDir in enumerate(imgDirs):
        baseDir = f"D:\\IndeVFX\pythonProject\\4D_point_cloud\\Meshroom_models\\3D_Models_for_frames\\Frame_{idx+1}"
        process_folder(binPath, baseDir, imgDir)
